...
class KVPairCocoManifestAdaptor(CocoManifestWithMultiImageAnnotationAdaptor):
    ...
    def process_label(self, ann_manifest: AnnotationDataManifest, annotation: dict, coco_manifest: dict):
        if not self._schema_match(annotation['key_value_pairs'], self._schema.get('fieldSchema', {})):
            raise ValueError(f"Annotation key_value_pairs does not match schema: {annotation['key_value_pairs']}")
        ann_manifest.label = KVPairLabelManifest(label_data={'key_value_pairs': annotation['key_value_pairs'], 'text_input': annotation.get('text_input', None)},
                                                 additional_info=self._get_additional_info(annotation, {'id', 'image_id', 'key_value_pairs', 'text_input'}))
