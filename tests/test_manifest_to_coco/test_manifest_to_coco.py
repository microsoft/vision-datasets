...
class TestManifestToCoco:
    @pytest.mark.parametrize("coco_dict, task", [(coco_dict, task) for task, coco_dicts in coco_database.items() if task not in [DatasetTypes.MULTITASK, DatasetTypes.KV_PAIR] for coco_dict in coco_dicts])
    ...
    def test_manifest_to_coco_dict_kvpair(self, coco_dict, schema):
        manifest = coco_dict_to_manifest(DatasetTypes.KV_PAIR, coco_dict, schema)
        coco_generator = CocoDictGeneratorFactory.create(DatasetTypes.KV_PAIR)
        coco_dict = coco_generator.run(manifest)