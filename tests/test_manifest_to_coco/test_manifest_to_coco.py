...
class TestManifestToCoco:
    @pytest.mark.parametrize("coco_dict, task", [(coco_dict, task) for task, coco_dicts in coco_database.items() if task not in [DatasetTypes.MULTITASK, DatasetTypes.KEY_VALUE_PAIR] for coco_dict in coco_dicts])
