...
class TestSampleManifestNumSamples:
    @pytest.mark.parametrize("task, coco_dict, with_replacement",
                             [x[0] + (x[1],)for x in itertools.product([(task, coco_dict) for task, coco_dicts in coco_database.items() if task != DatasetTypes.MULTITASK for coco_dict in coco_dicts], [True, False])])
