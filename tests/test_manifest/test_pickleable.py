...
class TestManifestIsPickleable:
    @pytest.mark.parametrize("task, coco_dict", [(task, coco_dict) for task, coco_dicts in coco_database.items() if task != DatasetTypes.KV_PAIR for coco_dict in coco_dicts])
