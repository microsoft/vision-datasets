...
def coco_dict_to_manifest(adaptor, coco_dict, data_type):
    with tempfile.TemporaryDirectory() as temp_dir:
        dm1_path = pathlib.Path(temp_dir) / 'coco.json'
        dm1_path.write_text(json.dumps(coco_dict))
        return adaptor.create_dataset_manifest(str(dm1_path), data_type)
...
coco_database = {
    DatasetTypes.IMCAP: ImageCaptionTestCases.manifest_dicts,
    ...
    DatasetTypes.IC_MULTICLASS: MultiClassClassificationTestCases.manifest_dicts,
    DatasetTypes.IC_MULTILABEL: MultiLabelClassificationTestCases.manifest_dicts,
    DatasetTypes.OD: ObjectDetectionTestCases.manifest_dicts,
    DatasetTypes.IMAGE_RETRIEVAL: Text2ImageRetrievalTestCases.manifest_dicts
}
